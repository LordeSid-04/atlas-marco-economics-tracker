...
function RiskPlaybookProbabilityGraph({ items, activeId, onSelect }) {
  if (!items.length) {
    return (
      <div className="rounded-lg border border-white/12 bg-white/[0.03] px-2.5 py-2 text-[12px] text-zinc-400">
  ...
  return (
    <div className="rounded-xl border border-white/12 bg-black/30 p-2.5">
      <div className="mb-2 text-[11px] uppercase tracking-[0.1em] text-zinc-500">Probability Trail (Interactive)</div>
  ...
  return (
    <div className="pointer-events-none fixed bottom-5 right-4 z-[65] flex max-w-[94vw] flex-col items-end gap-3">
          ...
          >
            ...
            {selectedProof ? (
              <div className="space-y-3">
                <div>
                  <div className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Executive Readout</div>
                  <div className="mt-1 text-sm leading-relaxed text-zinc-200">{selectedProof.conclusion.story}</div>
                  <div className="mt-1 text-[12px] leading-relaxed text-zinc-400">{selectedProof.conclusion.why_now}</div>
                </div>

                <div className="border-t border-white/10 pt-3">
                  <div className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Source Evidence</div>
                  <div className="mt-1 max-h-[180px] space-y-2 overflow-auto pr-1">
                    {(selectedProof.source_evidence || []).slice(0, 4).map((source) => (
                      ...
                      >
                        <div className="text-[12px] text-zinc-200">{source.title}</div>
                ...
                <div className="grid grid-cols-1 gap-3 border-t border-white/10 pt-3 sm:grid-cols-2">
                  <div>
                    <div className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Signal Validation</div>
                    {(selectedProof.model_evidence || []).slice(0, 1).map((row, idx) => (
                      <div key={`signal-validation-${idx}`} className="mt-1 text-[12px] leading-relaxed text-zinc-300">
                  ...
                  <div>
                    <div className="text-[11px] uppercase tracking-[0.12em] text-zinc-500">Market Signals</div>
                    <div className="mt-1 space-y-1">
                      {(selectedProof.market_evidence || []).slice(0, 2).map((item, idx) => (
                        <div key={`${item.signal}-${idx}`} className="text-[12px] leading-relaxed text-zinc-300">
              ...
              <div className="text-sm text-zinc-500">Proof data unavailable.</div>
            )}
          </motion.div>
        ) : null}
      </AnimatePresence>
      ...
      <button
        type="button"
        onClick={onToggle}
        className="pointer-events-auto atlas-focus-ring inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/35 px-4 py-2.5 text-sm font-medium uppercase tracking-[0.12em] text-zinc-100 transition hover:bg-black/55"
      >
...
function CountryIntelCard({ country, intelRows, healthySources, totalSources, countryProof, isLoadingProof, onClose }) {
  ...
  return (
    <div className="absolute bottom-5 right-5 z-[1300] w-[min(430px,calc(100%-2.5rem))] overflow-hidden rounded-2xl border border-white/15 bg-black/70 p-4 shadow-[0_24px_54px_rgba(0,0,0,0.55)] backdrop-blur-xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[11px] uppercase tracking-[0.14em] text-zinc-500">Country Intelligence</div>
      ...
      <div className="mt-3 rounded-lg border border-white/12 bg-white/[0.03] p-2.5">
        <div className="text-[11px] uppercase tracking-[0.11em] text-zinc-500">Reliable Feed Snapshot</div>
      ...
      <div className="mt-3">
        <div className="text-[11px] uppercase tracking-[0.11em] text-zinc-500">Latest News Evidence</div>
        <div className="mt-1.5 max-h-[190px] space-y-2 overflow-auto pr-1">
          {intelRows.length ? (
            intelRows.map((item) => (
              ...
              >
                <div className="line-clamp-2 text-[12px] text-zinc-100">{item.title}</div>
                <div className="mt-1 text-[11px] text-zinc-500">
            ...
            <div className="text-[12px] text-zinc-500">No verified country-specific source rows yet.</div>
          )}
        </div>
      </div>

      <div className="mt-3 border-t border-white/10 pt-3">
        <div className="text-[11px] uppercase tracking-[0.11em] text-zinc-500">Backend Data Proof</div>
        {isLoadingProof ? (
          <div className="mt-1 text-[12px] text-zinc-500">Loading source provenance...</div>
        ) : (
          <div className="mt-1 space-y-1">
            <div className="text-[12px] text-zinc-400">{countryProof?.methodology || "Deterministic proof unavailable."}</div>
            {(countryProof?.sources || []).slice(0, 3).map((source) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block text-[12px] text-zinc-300 transition hover:text-zinc-100"
              >
                {source.name}
              </a>
            ))}
      ...
      <div className="mx-auto max-w-[1650px] space-y-4">
        ...
        <section className={`${PANEL_CLASS} space-y-4 bg-black/24 p-3 backdrop-blur-sm sm:p-4`}>
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <h2 className="text-2xl font-semibold tracking-tight text-zinc-100 sm:text-3xl">Critical Developments</h2>
            </div>
          ...
          {selectedDevelopment ? (
            <div className="space-y-4 rounded-xl bg-white/[0.03] p-3 sm:p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="max-w-4xl">
                  <div className="text-[12px] uppercase tracking-[0.14em] text-zinc-500">Critical Developments</div>
                  <KeywordHighlighter
                  ...
                  <KeywordHighlighter
                    ...
                    className="mt-2 text-base leading-relaxed text-zinc-300"
                  />
                  {isFetchingDevelopmentDetail ? <div className="mt-1 text-[12px] text-zinc-500">Refreshing detailed intelligence...</div> : null}
                </div>
                <a
                  href={scenarioTarget}
                  className="inline-flex items-center gap-1.5 rounded-full border border-white/25 px-3.5 py-2 text-[12px] font-medium text-zinc-100 transition duration-300 hover:border-white/40 hover:tracking-[0.02em]"
                >
              ...
              <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                <div className="rounded-xl bg-white/[0.02] p-3">
                  <div className="mb-2 flex items-center gap-2 text-base font-semibold text-zinc-100">
                    <BookOpenText className="h-4 w-4 text-zinc-300" />
                    What This Means
                  </div>
                  <div className="text-[11px] uppercase tracking-[0.1em] text-zinc-500">Concise takeaways with practical next steps</div>
                    ...
                    {whatThisMeansItems.length ? (
                      whatThisMeansItems.map((item) => (
                        <div key={item.id} className="rounded-lg border border-white/12 bg-white/[0.04] px-2.5 py-2">
                          ...
                          <div className="mt-1 text-[12px] text-zinc-300">{item.insight}</div>
                          <div className="mt-1 rounded-md border border-cyan-300/25 bg-cyan-300/10 px-2 py-1.5 text-[11px] text-cyan-100">
                            Action: {item.action}
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="rounded-lg border border-white/10 bg-white/[0.02] px-2.5 py-2 text-[12px] text-zinc-400">
                    ...
                    {riskPlaybookItems.length ? (
                      riskPlaybookItems.map((item) => (
                        ...
                        >
                          ...
                          <div className="mt-1 text-[12px] text-zinc-300">{item.insight}</div>
                          <div className="mt-1 flex items-start gap-1.5 text-[11px] text-zinc-200">
            ...
            <div className="text-base text-zinc-500">Select a development to view analysis.</div>
          )}
        </section>
        {isLoading ? <div className="text-sm text-zinc-500">Loading live signal desk...</div> : null}
        {isError ? <div className="text-sm text-rose-300">Failed to load briefing: {error?.message || "Unknown error"}</div> : null}
      </div>
